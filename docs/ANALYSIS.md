...
- **주의(방법론적 한계):** 지점별 분위수 방식은 정의상 각 지점에서 상위 5%를
  ...
  산출하는 방식이 필요(→ **실험 ⑫에서 수행**). 이 실험의 의의는 "임계값은
  지점(군집) 단위로 산출해야 한다"는 설계 원칙의 확인이며, 이는 논문의 군집별
  모델 접근과 일관됨.
...
## 개선 실험 ⑪: 하이브리드 스코어 — 규칙 점수 + LSTM-AE 재구성 오차

실험 ④의 교훈("하이브리드(규칙+학습) 구조가 실용적 방향")을 검증
(`6_evaluation/hybrid_score_eval.py`). 규칙 점수(A2+D3 6특징 z-max)와 LSTM-AE
재구성 오차(실험 ③ 최고 구성: 지점별 정규화 + 위치·속도, max 집계)를 각각
지점별 학습 정상 통계로 z-정규화한 뒤 결합. 확대 평가셋(280개)으로 평가.

| 구성 | PR-AUC | Precision | Recall | F1 | 역주행 | 차로횡단 | 급정거 | 지그재그 |
|---|---|---|---|---|---|---|---|---|
| 규칙 단독 (D3) | 0.951 | 0.96 | 0.70 | 0.81 | 100% | 46% | 94% | 40% |
| AE 단독 | 0.886 | 0.92 | 0.48 | 0.63 | 36% | 13% | 74% | **70%** |
| 하이브리드 max | 0.961 | 0.95 | 0.74 | 0.83 | 100% | 43% | 91% | 63% |
| **하이브리드 mean** | **0.966** | 0.95 | **0.76** | **0.85** | 100% | 46% | 89% | **70%** |

![Hybrid score evaluation](analysis/17_hybrid_score_eval.png)

**상보성 분석 — 왜 결합이 이기는가:**

- 두 방식의 유형 프로필이 정확히 반대: 규칙은 역주행 100%·지그재그 40%,
  AE는 역주행 36%·지그재그 70%. **규칙이 놓친 이상 84개 중 AE가 38개 회수**
  (지그재그 42개 중 28개, 급정거 4개 중 3개). AE의 재구성 오차는 고주파 사행
  패턴에 크게 반응 — 규칙의 osc 특징이 부분적으로만 잡던 영역.
- 점수 공간 산점도에서 정상은 원점 부근에 뭉치고, 이상은 규칙 축 또는 AE 축
  한쪽으로 뻗음 — 두 점수가 서로 다른 이상 축을 측정한다는 직접 증거.
- 비용: AE가 추가하는 오탐은 정상 150개 중 11개 → 하이브리드 FP 8→11,
  Precision 0.96→0.95로 소폭 하락. Recall 이득(0.70→0.76)이 훨씬 큼.
- mean이 max보다 나은 이유: max는 한쪽 점수의 꼬리 노이즈를 그대로 통과시키고,
  mean은 양쪽 증거를 요구해 단독 노이즈를 절반으로 희석.

**채택:** 최종 구성은 **A2+D3+하이브리드 mean (F1 0.85 / PR-AUC 0.966)**.
실험 ④에서 "규칙이 AE를 이긴다"였던 결론이 "규칙이 주도하되 AE가 지그재그
계열을 보완한다"로 정밀화됨 — 학습 모델의 자리는 대체가 아니라 보완이었다.

## 개선 실험 ⑫: EVT(POT) 기반 동적 임계값 — "상위 X%" 임의 기준 제거

실험 ②에서 예고한 후속 과제(`6_evaluation/evt_threshold_eval.py`). 지점별 95%
분위수 임계값(①②⑪)은 두 가지 문제가 있다: (a) "5%"라는 기준 자체에 통계적
근거가 없고, (b) 분위수는 꼬리 표본 몇 개의 위치에 그대로 좌우된다.

**방법 — Peaks-Over-Threshold (Siffer et al., KDD 2017):** 학습 정상의
하이브리드 mean 점수(⑪ 채택 구성)에서 앵커 u(90% 분위수) 초과분에 일반화
파레토 분포(GPD)를 MLE 적합하고, 목표 초과확률 q에 대한 임계값을 해석적으로 산출:

> z_q = u + (σ/γ)((q·n/Nu)^(−γ) − 1)

q는 "정상 궤적이 임계값을 넘을 확률", 즉 **운영자가 허용하는 오경보율**이라는
직접적 의미를 가진다 — 임의의 "상위 X%" 대신 통제 가능한 파라미터.

**지점별 GPD 적합** (앵커 u = 학습 정상 90% 분위수):

| 지점 | n (학습 정상) | Nu (초과분) | γ (형상) | σ (척도) |
|---|---|---|---|---|
| 11 | 354 | 36 | +0.27 (두꺼운 꼬리) | 0.81 |
| 14 | 164 | 17 | −0.55 (유계 꼬리) | 1.03 |
| 17 | 86 | 9 | −0.99 (유계 꼬리) | 1.69 |

**결과 — 분위수 vs POT** (확대 평가셋, 하이브리드 mean 고정):

| 구성 | Precision | Recall | F1 | FP | 역주행 | 차로횡단 | 급정거 | 지그재그 |
|---|---|---|---|---|---|---|---|---|
| 95% 분위수 (⑪ 기준) | 0.95 | **0.76** | **0.85** | 11 | 100% | 46% | 89% | 70% |
| **POT q=0.05** | **0.97** | 0.74 | 0.84 | **7** | 100% | 40% | 86% | 69% |
| POT q=0.01 | 0.97 | 0.63 | 0.76 | 6 | 96% | 29% | 81% | 46% |
| POT q=0.001 | 0.96 | 0.44 | 0.61 | 5 | 83% | 6% | 70% | 19% |

![EVT threshold evaluation](analysis/18_evt_threshold_eval.png)

**해석:**

- **탐지 성능은 동급** (F1 0.85 vs 0.84). POT q=0.05는 같은 명목 수준에서
  분위수보다 임계값을 약간 높게 잡아(꼬리 모델이 표본 밖 극단값을 반영)
  **오탐 11→7개(36% 감소)**, Recall −0.02의 교환. q를 더 낮추면 Precision은
  이미 포화라 Recall만 잃는다 — 이 실험의 가치는 성능이 아니라 **임계값
  설정의 근거**에 있다.
- **앵커 둔감성 확인 (핵심 이득):** 분위수 방식에서 기준을 85→95%로 바꾸면
  임계값이 0.69~1.15 이동하지만, POT z_q(q=0.01 고정)는 앵커 u를 85/90/95%로
  바꿔도 0.16~0.24만 이동 (지점 17: 변동폭 1.151 vs 0.161, **7배 안정**).
  "몇 %를 자를 것인가"라는 임의 선택이 결과를 지배하지 않게 됨.
- **운영적 의미:** q는 지점·교통량과 무관한 목표 오경보율로 통일 설정 가능.
  전 지점(39개) 확장 시 지점마다 분위수 기준을 재조정할 필요 없이 q 하나로
  임계값이 자동 결정된다 — 동적 임계값의 실질적 가치는 확장 시나리오에서 발생.
- **주의(소표본 한계):** 지점 17은 초과분 Nu=9로 GPD 적합이 불안정
  (γ=−0.99는 경계 근처 추정). 꼬리 적합 진단(왼쪽 패널)에서 지점 11(γ>0)은
  경험 분포와 잘 일치하나, 지점 14·17은 표본이 적어 신뢰구간이 넓다.
  전 지점 확장으로 지점당 표본이 늘면 적합 신뢰도가 개선될 것.

**판정:** ⚠️ **조건부 채택** — 탐지 성능 개선은 아니나(F1 동급), "상위 5%"
임의 기준을 오경보율 q라는 통제 파라미터로 대체하고 앵커 선택 민감도를
3~7배 줄임. 기본 구성은 95% 분위수를 유지하되, 오탐 비용이 큰 운영 환경과
전 지점 확장 시 POT(q=0.05)를 권장 대안으로 채택.


## 개선 실험 ⑬: 예측 기반 이상 점수 — AE 과잉일반화 회피 시도 (부정적 결과)

ROADMAP 4-3의 미결 항목(`6_evaluation/prediction_score_eval.py`). 재구성(AE)은
이상 궤적까지 "잘 복원"해버리는 과잉일반화가 알려진 약점이므로(③에서 실측),
과거 40스텝에서 미래 10스텝을 맞혀야 하는 예측(seq2seq) 오차가 행동 변화
(급정거·사행·횡단 개시)에 더 민감하리라는 가설을 검증했다. 변인 통제를 위해
모델 규모(LSTM 64)·에폭(10)·시드(42)·윈도 생성은 ⑪의 AE와 동일하게 고정.

**결과 — 확대 평가셋 (지점·유형당 24개, 학습 정상 q95 보정):**

| 구성 | PR-AUC | F1 | FP | 역주행 | 차로횡단 | 급정거 | 지그재그 |
|---|---|---|---|---|---|---|---|
| AE 단독 | 0.886 | 0.63 | 12 | 36% | 13% | 74% | 70% |
| 예측기 단독 | 0.848 | 0.52 | 11 | **1%** | 17% | 77% | 49% |
| 하이브리드 mean (⑪ 채택) | 0.966 | **0.85** | 11 | 100% | 46% | 89% | 70% |
| 규칙+예측 mean | 0.961 | 0.83 | 13 | 100% | 51% | **96%** | 53% |
| 규칙+AE+예측 mean | 0.960 | 0.82 | 14 | 99% | 46% | 86% | 61% |
| 규칙+max(AE,예측) | 0.964 | 0.83 | 11 | 100% | 49% | 89% | 60% |

![Prediction score evaluation](analysis/19_prediction_score_eval.png)

**해석:**

- **예측기는 AE를 이기지 못한다** (단독 0.52 vs 0.63; 어떤 결합도 ⑪의 0.85 미달).
  결정적으로 두 점수의 상관이 r=0.86 — 예측기는 대체로 **AE와 같은 이상을
  본다**. "다른 관점"이라는 가설 자체가 성립하지 않았다.
- **역주행 1%의 교훈:** 시간을 반전한 궤적도 국소 다이나믹스(다음 10스텝)는
  매끄럽게 이어지므로 예측 오차가 나지 않는다. 예측은 "궤적이 물리적으로
  자연스러운가"를 볼 뿐, "차로 방향과 맞는가"는 규칙(방향장 정렬)만이 본다.
- **유형별 강점은 실재하나 상쇄된다:** 예측기는 급정거(77%, 규칙+예측 결합 시
  96%)와 차로횡단(+5%p)에 강점이 있지만, 지그재그에서 AE(규칙 미탐 42개 중
  28개 회수)의 절반(14개)밖에 회수하지 못해 총합에서 밀린다. 고주파 사행은
  "예측하기 어려운" 것이 아니라 "재구성 매니폴드에서 먼" 패턴이었다.
- 3-점수 융합(mean/max)도 이득 없음 — 예측기의 회수분(26개)이 AE 회수분(38개)과
  35개 겹쳐, 추가 회수 3개 대비 오탐 희석 손실이 더 크다.

**판정:** ❌ **기각** — 예측 오차는 AE 재구성 오차의 대체재도 보완재도 되지
못한다(상관 0.86, 회수 부분집합). 단, "예측기는 시간 구조 이상(역주행)에
원리적으로 무반응"이라는 발견은 점수원 설계의 교훈으로 남는다: 이상 점수는
**어떤 불변량을 위반하는지**로 선택해야 하며, 물리적 자연스러움(예측)과
패턴 전형성(재구성)과 차로 정합성(규칙)은 서로 다른 불변량이다.

## 개선 실험 ⑭: 융합 위상 × 운영점 — "F1 0.88"의 함정과 진짜 여유

⑬의 점수 덤프로 융합 구조를 재학습 없이 탐색하던 중
(`6_evaluation/fusion_operating_point_eval.py`), 결정 융합
**OR(규칙,AE,예측 — 각자 q95 보정 후 합집합)이 F1 0.88**로 ⑪(0.85)을 넘는
것을 발견했다. 그러나 오탐이 11→22로 두 배였고, 이는 공정한 비교가 아니다.

**오탐 매칭 비교 (같은 오경보 예산에서 구조끼리 비교):**

| FP | 점수 융합 (hybrid mean, q 조정) | 결정 융합 OR(r,a,p) |
|---|---|---|
| 22 | **0.90** (q87) | 0.88 (q95) |
| 19 | **0.89** (q89) | 0.87 (q96) |
| 14 | **0.88** (q92.5) | 0.84 (q97) |
| 12 | **0.85** (q95) | 0.82 (q98) |
| 8 | **0.83** (q95.5) | 0.80 (q99) |

![Fusion operating point](analysis/20_fusion_operating_point.png)

**해석:**

- **모든 매칭 지점에서 점수 융합(mean)이 결정 융합(OR)을 지배한다.** OR의
  "개선"은 구조의 이득이 아니라 오경보 예산을 더 쓴 것이었다. 연속 점수를
  이진 결정으로 일찍 붕괴시키면 정보가 손실된다 — 늦게 결합할수록 좋다.
- **진짜 발견은 운영점 곡선이다:** ⑪ 점수 그대로 q만 풀면 q90에서 F1 0.89
  (R 0.85, FPR 11%), q85에서 **F1 0.92 (R 0.93, 차로횡단 79%, 지그재그 91%,
  FPR 18%)**. 점수가 아니라 명목 오경보율 5%(q95)라는 운영점이 Recall을
  묶고 있었다. 차로횡단 "46% 한계"의 상당 부분은 임계값 바로 아래에 있었다.
- **단, q를 평가셋 F1로 고르면 그것도 누수다.** 채택 구성(q95)은 유지하고,
  이 곡선은 "오경보 비용 대비 탐지율" 운영 결정 자료로 기록한다. ⑫(POT)의
  q=오경보율 해석과 결합하면 운영자가 비용 기반으로 운영점을 고를 수 있다.

**판정:** ✅ **방법론 채택 (구성 변경 없음)** — 융합은 점수 수준(mean)이 옳고
(⑪ 유지), 이후 모든 임계값 비교는 오탐 매칭 위에서 수행한다. 운영점 곡선을
표준 산출물로 추가 (오경보 예산 1%p당 Recall 약 1.5%p의 교환비).

## 개선 실험 ⑮: AE 학습 예산 — "겉보기 개선"의 두 번째 사례 (부정적 결과)

ROADMAP 4-1의 미결 항목(`6_evaluation/ae_training_budget_eval.py`). ③부터
유지해 온 AE 10 에폭 관례는 검증용이었고 val_loss가 수렴 전이므로, 학습을
연장하면 하이브리드의 AE 축이 개선될 것이라는 상식적 가설을 검증했다.
⑪ 구성 고정, AE만 10 → 30 → 60 에폭 연장.

**결과 — 고정 q95만 보면 명백한 개선처럼 보인다:**

| 에폭 | val_loss | PR-AUC | F1@q95 | FP | 차로횡단 | 급정거 | 지그재그 |
|---|---|---|---|---|---|---|---|
| 10 (⑪) | — | 0.966 | 0.85 | 11 | 46% | 89% | 70% |
| 30 | 0.00013 | 0.968 | 0.86 | 13 | 49% | 96% | 74% |
| 60 | 0.00010 | **0.972** | **0.89** | **16** | 57% | 99% | 84% |

그러나 ⑭의 원칙대로 오탐을 매칭하면: 60ep을 FP 10으로 조이면(q97.5)
**F1 0.84·R 0.75 — 10ep(0.85)과 동급**. 세 학습 예산의 운영점 곡선은
사실상 겹치며(왼쪽 패널), 고정 q95 별표가 같은 곡선 위에서 오른쪽으로
미끄러졌을 뿐이다.

![AE training budget](analysis/21_ae_training_budget.png)

**메커니즘:** 학습이 길어질수록 AE가 학습 정상을 더 정밀하게 암기 →
학습 정상의 재구성 오차 분포가 좁아짐 → 같은 q95 임계값이 절대값 기준으로
낮아짐 → 평가 정상(일반화 갭)의 z-점수가 상대적으로 부풀며 오탐 증가.
즉 "학습 연장"은 점수 개선이 아니라 **임계값 완화의 우회 경로**였다.
점수 자체의 품질(PR-AUC)은 +0.006로, ⑨에서 확립한 소표본 노이즈 범위를
넘지 못한다.

**판정:** ❌ **기각 (관례 10 에폭 유지)** — 단, 이 실험의 가치는 ⑭에서 세운
"오탐 매칭 없는 비교는 무의미" 원칙의 독립적인 재실증에 있다. OR 융합(⑭)과
학습 연장(⑮), 서로 무관해 보이는 두 "개선"이 모두 같은 착시(운영점 이동)로
환원됐다 — 매칭 비교를 표준 절차로 삼지 않았다면 둘 다 채택됐을 것이다.
